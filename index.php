...
Cross-Site Request Forgery (CSRF) is a type of attack that tricks a user's browser into executing unwanted actions on a web application in which they're authenticated. 
This can lead to unauthorized actions such as changing account details, making purchases, or even transferring funds.
To mitigate this risk, implementing CSRF protection is crucial. One common method is to use CSRF tokens. 
A CSRF token is a unique, secret, and unpredictable value that is generated by the server and included in forms and requests. 
When a form is submitted, the server checks if the token is valid and matches the one stored in the user's session. 
If the token is missing or incorrect, the request is rejected.
In this code snippet, we generate a CSRF token if it doesn't already exist in the user's session. 
This token should then be included as a hidden field in any forms that perform state-changing operations (like login, registration, etc.). 
When the form is submitted, the server should validate the token to ensure the request is legitimate.
*/
...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if(empty($username_err) && empty($password_err) && empty($confirm_password_err)){
        ...
        if($stmt = mysqli_prepare($link, $sql)){
            ...
            /* Revised: 09-11-2025 
                Password Hashing
                Password hashing is a critical security measure used to protect user passwords in databases.
                Instead of storing plain text passwords, which can be easily compromised, passwords are transformed
                into a fixed-length string of characters using a hashing algorithm. This process is one-way, meaning
                that once a password is hashed, it cannot be easily reversed back to its original form

                PHP provides a built-in function called password_hash() that simplifies the process of hashing passwords.
                This function uses strong hashing algorithms and automatically handles the generation of a unique
                salt for each password, which adds an additional layer of security.
            */
